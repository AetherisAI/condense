...
- **Agent collect exclude-file-patterns:** `agent/sync.py`'s existing `DEFAULT_EXCLUDE_DIRS`
  ...
  closes the Leitat-corpus finding where agent-internal bookkeeping files polluted the index.
...
### E2E — Acceptance (against the Leitat corpus, real LLM)
...
- [x] (c) An ambiguous query triggers visible multi-tool probing (multiple `tool_call`/
      ...
      time (e.g. `(Hiring/Xavier-Sabate.pdf, p.1)`), `truncated: false`. One `tool_call`/
...
- [x] Citation format tightened in `pipelines/answer.py._SYSTEM_PROMPT`: exact shape spelled out
      ...
      (`.../Angel.pdf,1.` fused into a path). New
...
**Trigger:** a live self-test against the real Leitat corpus (50 docs, ingested one-shot then
...
- [x] Live verify (fresh `sift-engine-wp2` restart, real Mistral, real 50-doc Leitat corpus,
...
snippet-truncation mismatch between `pipelines/search.py`/`pipelines/tools.py`, and a real Leitat
...
real 82KB Leitat budget spreadsheet came back with ~2,900 literal `"NaN"` occurrences, diluting
...
- [x] **Acceptance evidence — the two real motivating files, parsed directly (no live-DB
      writes):** `AT-FR0198_Anexo-II_Ppto_Subc_NACIONAL_Rev.0(1).xlsx` and `...Rev.03.xlsx`
      (`/home/quentinlatimier/Documents/Leitat/DNOTA-DIGITOOL/`) both now parse with **zero**
      `"NaN"` occurrences anywhere in the extracted text (was 2,904 / 2,994 respectively before
      the fix — measured directly, not estimated), while real content (`"PERSONAL"`, `"PPTO"`

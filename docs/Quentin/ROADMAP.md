...
> ✅ **2026-07-08 — public-hardening/audit wave MERGED; desktop WP COMPLETE, awaiting validation.** Repo went public (MIT, D68); a tri-review + three follow-on PRs (#22–#25) landed on `main`: `/status` secret redaction + constant-time auth + CI (SAST/web/quality), loopback-only compose bind by default (D70), master-gated `/v1/tokens` mint/list/revoke (D69), core `SearchOutcome` + AST layering test + `chunk_tokenizer=auto` (D72) — suite 500→518 green. Supply-chain hardening (D71) done locally, push pending a token-scope fix. In parallel, `feat/desktop-standalone` (D60–D67: Tauri shell, PyInstaller server bundle, llama.cpp local embeddings) is feature-complete — Quentin is validating an installed build; merge tags **v0.4.0** on his word.
>
...
> ✅ **2026-07-06 — v0.3.0 MERGED.** Chat-first "workbench" UX (D56/D57: Ask|Find modes, composer ingest, simple-first System, left Library, logo-as-status) + api.ts/CORS foundations + agent `--json`/SIGTERM + headless sidecar binary + Arthur's landing page (`site/`, PR #21) + health-probe & pyright-drift fixes, shipped to `main` (42ee516, tag v0.3.0). Archive: `docs/Quentin/archive/v0.3.0-ux-workbench/`. Next WP: revise + implement `feat/tauri-shell` (D53-amended: provisioning end-state).
>
> ✅ **2026-07-05 — v0.2.0 MERGED.** LLM-agnostic toolbox + `/v1/answer` chat agent + grounding modes (strict/hybrid/open) + rich markdown chat, shipped to `main` (55b52b4, tag v0.2.0). D37–D51. Archive: `docs/Quentin/archive/v0.2.0-toolbox-answer/`. Next WP: Tauri desktop app + ingest agent from UI.

> ✅ **2026-07-04 — v0.1.0 MERGED.** Dev B surface rebuilt async on Arthur's foundation and shipped to `main` (squash 8966686): version-aware retrieval (mtime collapse, D28/D33), RAM-safe agent+engine chain (D29/D31/D32/D34), local TEI embeddings (D30), SiftError-terminal fallback (D36). Docs: `docs/Quentin/archive/v0.1.0-version-aware-retrieval/`.
>

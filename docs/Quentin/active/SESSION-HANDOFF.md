...
A real-load E2E run (real TEI embed backend, real Acme corpus) hit TEI OOM-thrashing under its
...
Both `.docx` parsed fine in ~1.5s. The `.xlsx` (`project-schedule_2026.xlsx`,
...
- **R4 (vendored-directory exclusion):** `agent/sync.py`'s walk (`_iter_matching`, shared by
  ...
  Acme corpus finding: numpy/lxml license files nested under
  `ACME-TOOLING/.venv/lib/site-packages/*.dist-info` were being uploaded as if they were the
...
real Acme `.xlsx` files), plus three smaller hardening items found in the same pass (all TDD,
failing-first):
...
- **The gate miss (`OcrFallbackParser` exception scoping):** `adapters/ocr/fallback_parser.py`'s
  ...
  Reproduced 3× on both real schedule `.xlsx` files. **Fix:** a new `except SiftError: raise`
...
1. **E2E v3 — what it must prove** (the reason this whole audit/fix wave exists): re-run the
   real-Acme-corpus ingest against the now fully-hardened stack (bounded embed/OCR timeouts,

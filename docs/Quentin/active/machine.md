...
| 2026-07-06 | (D67, spinner bug) | **Owner's real first launch (GNOME/Wayland, ~09:09) stuck on the animated-logo loading screen forever — root-caused, fixed, re-verified (D67).** Root cause: `SetupWizard.tsx`'s init effect `Promise.all([appConfigGet(), provisioningStatus()])` had no rejection path and the bare busy logo rendered until it resolved; on a pristine config the Rust `provisioning_status` fetches the baked default manifest URL (raw.githubusercontent.com, PRIVATE repo) → HTTP 404 in ~0.3s (curl-verified from the repro container) → unhandled rejection → `setLoading(false)` never ran. Masked by every prior E2E (pre-seeded `file://` manifest_url) AND by browser QA (the `tauri.ts` mock never rejects). Repro matrix under `~/.cache/condense-desktop-assets/spinner-bug/` (isolated container HOMEs, frozen pre-fix build `3e42c92d…`): run1 pristine+internet = stuck at 10s/30s/90s, WebKitWebProcess ~38% CPU (the `.mark-busy` CSS loop — explains the owner's 4m50s/12min); run2 `file://` manifest = choice screen at 10s. **Fix:** `SetupWizard.tsx` — config load decoupled from `provisioning_status` (choice screen never gated on the manifest fetch; manifest failure = inline error + Retry on the Run-locally step with copy pointing at System ▸ Desktop; `app_config_get` failure = error card; zero infinite spinners); `provisioning.rs` — `include_str!` embedded-manifest fallback when the DEFAULT URL fails (surfaced as `source:"embedded-fallback"` + wizard/System notice; user-set URLs still fail honestly), shared reqwest client with `connect_timeout(10s)` + 30s total cap on the manifest fetch (both previous `reqwest::get` calls had NO timeout at all); `SystemMenu.tsx` — manifest URL now editable (empty = default/null, Save persists + re-checks; Desktop-section loader also decoupled so a broken user URL can't hide the field that fixes it); `tauri.ts` — `localStorage.mockManifestFail='1'` QA seam so the mock can reject like the real command. **Gates:** web `tsc -b && vite build` + `oxlint` clean; container `cargo fmt --check` + `cargo check` clean; full `tauri build` green. **Re-verified on the fixed build (`e890e1df…`):** run3 pristine+internet = choice screen at 8s, Run locally shows the embedded-fallback list + notice (engine —/15.1MB/605.2MB); run4 `file://` manifest = unchanged (127.5MB real engine size, no notice); Chrome mock QA on a temp :5177 vite (this worktree) = fail→error+Retry→recover, manifest-URL field saves and re-disables. Installed `~/.local/bin/Condense.AppImage` AND the T9 kit refreshed (kit tar.gz rebuilt, `81c398b2…`); owner's stale `~/.config/ai.aetheris.condense` + `~/.local/share/ai.aetheris.condense` (config.json + webview storage only — verified before delete) wiped for a pristine next launch. Production untouched. |

## Decisions
- D60–D67 (this WP) — see `docs/Quentin/DECISIONS.md`.

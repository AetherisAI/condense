...
- [ ] Chrome QA gate: drop 2 files (1 good, 1 corrupt xlsx from Leitat) → ingest turn shows 1 indexed + 1 failed WITH the reason; toggle persists; old toggles gone.
...
- [ ] Chrome QA gate: "cronograma" in Find mode → clean list, top hit tinted, zero LLM latency; Ask mode unchanged.

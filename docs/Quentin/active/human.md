...
**Status:** COMPLETE — awaiting Quentin's review + merge word · **Branch:** `feat/desktop-standalone` · **Updated:** 2026-07-06 10:15
...
- **D67** — the owner's own first launch hit a third blocker: infinite spinner on every pristine install (unhandled `provisioning_status` rejection — the default manifest URL 404s while the repo is private). Fixed: choice screen no longer gated on the manifest fetch; embedded-manifest fallback (surfaced, not silent); reqwest timeouts; editable manifest URL in System ▸ Desktop.
...
- **D67 pristine-install spinner — FIXED**: the app now falls back to the embedded manifest (visible notice) instead of hanging. Engine downloads still 404 honestly until a real release exists — use the test kit. Owner's stale config/data dirs wiped; next launch is pristine on the fixed build (`e890e1df…`).

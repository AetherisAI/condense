...
## 2026-07-02 — A12: `/ingest` streams files one at a time (bounded peak RAM)  [WP: ingest]
- **Decision:** The `/ingest` route hands the pipeline a **lazy async generator** over the `UploadFile`s (read → yield → `close`) instead of `payload = [(name, await f.read()) for f in files]`. The ingest seam widens from `Sequence[tuple[str, bytes]]` to `IngestFiles = AsyncIterable | Iterable`, normalized by a new `stream_files()` helper; `IngestPipeline` and the `_StubIngest` fake iterate with `async for`. (A11 is the parallel `feat/agent-download` branch.)
- **Why:** The old route materialized **every** uploaded file's bytes into one list before ingesting, so peak RAM ≈ the sum of all files in the request — a folder ingest or a few 500-page PDFs spiked memory and could OOM the shared engine. Streaming keeps only one file (plus its chunks/vectors) resident at a time. The union type is backward-compatible: lists (all existing tests) still work, so no test churn beyond the new guard.
- **Alternatives:** Change the seam to `AsyncIterator` only — breaks every list-based test call. A sync generator reading `file.file.read()` — blocks the event loop (Starlette's `await file.read()` is threadpool-backed; keep it async). A hard max-upload-size cap — rejects legitimate large batches instead of handling them.
- **Basis:** Regression test `test_ingest_streams_files_one_at_a_time` (asserts ≤1 file resident; the buffering version drained all 5 up front); full suite 130 green; live multi-file `/ingest` still indexes. Touches Dev B's `api/routes.py` + the co-owned seam — flagged to Quentin.

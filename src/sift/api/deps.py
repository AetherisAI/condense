...
``ingest_token`` maps to ``"default"`` and anything else (or a missing header) is rejected 401.
Multi-tenancy stays additive — only this function changes when real tenants arrive.
"""
...
async def resolve_tenant(
    ...
    The configured ``ingest_token`` resolves to ``"default"``; a missing or wrong token is
    rejected 401 with a ``WWW-Authenticate: Bearer`` challenge.
    """
    if credentials is not None and credentials.credentials == container.settings.ingest_token:
        return "default"
    raise HTTPException(

...
class TokenChunker:
    ...
    ``chunk_overlap`` tokens. Each window is decoded back to text and stripped; empty
    windows are dropped. ``index`` is a document-global 0-based ordinal over the emitted
    chunks, and ``page`` is the page that the window's *start* token falls in (always the
    sole page's number under markitdown's single-page parsing). Pure-CPU and deterministic
    — the ``async def`` never awaits, so identical input yields identical chunks/indices.
    """
